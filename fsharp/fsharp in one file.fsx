...
type SimpleClass(age: int, name: string) =
...
let greaterOrNot x y = if x > y then "yes" else "no" // simple if-else
...
let compareNums x y = //  case when we have more possibilities
    match (x, y) with
    | 1, 2
    | 2, 1 -> "one and two" // we can choose from many cases with `|`
    | x, y when x > y -> "x greater than y"
    | x, y when x >= 100 && y >= 100 -> "both at least 100"
...
let divide x y =
    try
        ...
        if x < 0 then
            failwith "we don't accept negative values"
...
let lenghtPlusOne = mapAndAdd1 (fun x -> x.Length) "txt" // (fun) is a anonymous func a.k.a "lambda"
let parsePlusOne = mapAndAdd1 (fun x -> Int32.Parse x) "5"
...
let add1AndMultiplyBy5 x =
    ...
    let multiplied = x |> add1 |> multiplyBy5 // we can define chain of functions aka 'pipeline'

    let composed = add1 >> multiplyBy5 // '>>' it's a composition pipeline let (>>) f g x = g(f(x))
    let multiplied = x |> composed
    let multiplied = x |> (add1 >> multiplyBy5) // 3. with composition
...
let createReservation
    ...
    id
    =
    if resExists (id) then
        "not created"
    else
        codeGen (id)

// ------------- during startup resolve dependencies -----------------------
let codeGen (id: string) = $"SPECIAL_CODE:{id.Length}" // this is string interpolation

let reservationExists (id: string) = id.Length > 10
...
let subtract2AndMultiply3 = subtractXAndMultiplyY 2 3
let shouldBeSix = subtract2AndMultiply3 4
...
let myMap = Map.empty |> Map.add "Bartosz" 5
...
type MyRecord2 = { X: int; Y: string; Z: int64 }

let myRecord2 = { X = 15; Y = "other txt"; Z = 5L } // compiler takes the last matching record by default
...
type TryDivideError = | DivideByZero
...
// TODO resovle JsonProvider and HtmlProvider not found.
open FSharp.Data
...
//FsharpInfo().Tables.VersionsEdit.Rows |> ignore // we can access F# versions table from wiki
